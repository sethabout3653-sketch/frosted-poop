...
type Traverse<O extends Record<any, any>, P extends string> = P extends `${infer K}.${infer R}`
  ? Traverse<O[K], R>
  : O[P];
...
type ProxyApplyThis<T extends string> =
  unknown extends ThisParameterType<Extract<GlobalTraverse<T>, AnyFunction>>
    ? T extends `${infer ClassName}.prototype.${string}`
      ? GlobalTraverse<ClassName> extends {
...
export type ProxyCtx<T extends string = string, U extends "construct" | "apply" = "apply"> = {
  ...
  return: (
    r: IfEquals<U, "construct", InstanceType<GlobalTraverse<T>>, ReturnType<GlobalTraverse<T>>>,
  ) => void;
  call: () => IfEquals<
    U,
    "construct",
